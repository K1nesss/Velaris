...
<section class="app-page">
	<div class="app-page-inner">
		<header class="page-header">
			<div>
				<h1 class="page-title">{t('仪表盘', 'Dashboard')}</h1>
			</div>
		</header>
		...
		<div class="grid gap-6 xl:grid-cols-2">
			<div class="min-w-0 space-y-6">
				<div class="grid gap-6 sm:grid-cols-2">
					<div class="metric-card group p-6">
						<div class="relative z-10 flex items-center justify-between gap-4">
							<div>
								<h3 class="eyebrow mb-1">{t('今日时长', "Today's Playtime")}</h3>
								{#if initialLoading}
									<div class="skeleton-shimmer mt-2 h-10 w-36 rounded-lg sm:h-11"></div>
								{:else}
									<div class="metric-value text-3xl font-bold sm:text-4xl">{todayPlaytime}</div>
								{/if}
							</div>
							<div
								class="relative z-10 flex h-12 w-12 items-center justify-center rounded-2xl bg-(--pt-accent-soft) text-(--pt-accent)"
							>
								<svg
					...
					<div class="metric-card group p-6">
						<div class="relative z-10 flex items-center justify-between gap-4">
							<div>
								<h3 class="eyebrow mb-1">{t('本周时长', 'This Week')}</h3>
								{#if initialLoading}
									<div class="skeleton-shimmer mt-2 h-10 w-36 rounded-lg sm:h-11"></div>
								{:else}
									<div class="metric-value text-3xl font-bold sm:text-4xl">{weekPlaytime}</div>
								{/if}
							</div>
							<div
								class="relative z-10 flex h-12 w-12 items-center justify-center rounded-2xl bg-(--pt-accent-soft) text-(--md-primary)"
							>
								<svg
									xmlns="http://www.w3.org/2000/svg"
									viewBox="0 0 24 24"
				...
				<CurrentPlayingCard {currentPlaying} loading={initialLoading} {language} />
				<RecentSessionsList sessions={recentSessions} loading={initialLoading} {language} />
			</div>

			<div class="min-w-0 space-y-6">
				<DailyChart items={dailyChartItems} loading={initialLoading} {language} />
				<DonutChart items={donutItems} loading={initialLoading} {language} />
			</div>
		</div>
	</div>
</section>
