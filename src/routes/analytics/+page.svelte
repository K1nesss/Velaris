<script lang="ts">
	...
	function rangeIndex(days: number) {
		const index = RANGE_OPTIONS.findIndex((option) => option === days);
	...
	function buildTopGamesOption(data: NonNullable<AnalyticsSnapshot>['top_games']) {
		...
		return {
			...
			animationDuration: animationsEnabled ? 450 : 0,
	...
	function buildTrendOption(points: TrendPoint[]) {
		...
		const average =
			points.length > 0 ? points.reduce((sum, point) => sum + point.seconds, 0) / points.length : 0;
		...
		return {
			...
			tooltip: {
				...
				textStyle: { color: isDark ? '#e5e7eb' : '#111827' },
				formatter(
					params: Array<{ axisValue: string; data: number; marker: string; seriesName?: string }>
				) {
					const title = params[0]?.axisValue ?? '';
					const lines = params.map(
						(item) => `${item.marker}${item.seriesName ?? ''} ${formatDurationCompact(item.data)}`
					);
...
<div class="app-page h-full overflow-y-auto">
	<div class="app-page-inner pb-8">
		<header class="page-header">
			<div>
				<h1 class="page-title">{t('数据分析总览', 'Analytics Overview')}</h1>
			</div>

			<div
				class="segmented-control"
				style={`--segmented-index: ${rangeIndex(selectedDays)}; --segmented-count: ${RANGE_OPTIONS.length};`}
			>
		...
		{#if errorMessage}
			<div
				class="status-message mb-6 border-red-200 bg-red-50 text-red-700 dark:border-red-900/50 dark:bg-red-950/20 dark:text-red-300"
			>
				{errorMessage}
			</div>
		{/if}
		...
		<div class="mb-8 grid grid-cols-1 gap-6 md:grid-cols-3">
			<div class="metric-card relative flex items-center justify-between overflow-hidden p-6">
				<div class="relative z-10">
					<h3
						class="mb-2 text-xs font-semibold tracking-wider text-gray-500 uppercase dark:text-gray-400"
					>
						{t('总游玩时长', 'Total Playtime')}
					</h3>
					{#if loading || !snapshot}
						<div class="skeleton-shimmer h-8 w-24 rounded-md"></div>
					{:else}
						<div class="text-2xl font-bold text-gray-900 dark:text-white">
							{snapshot.summary.total_formatted}
						</div>
						<div
							class="mt-1 inline-flex items-center rounded px-2 py-0.5 text-xs font-medium {snapshot
				...
				<div
					class="relative z-10 flex h-12 w-12 items-center justify-center rounded-full bg-purple-100 dark:bg-purple-900/30"
				>
					<svg
			...
			<div class="metric-card relative flex items-center justify-between overflow-hidden p-6">
				<div class="relative z-10">
					<h3
						class="mb-2 text-xs font-semibold tracking-wider text-gray-500 uppercase dark:text-gray-400"
					>
						{t('平均会话时长', 'Average Session')}
					</h3>
					{#if loading || !snapshot}
						<div class="skeleton-shimmer h-8 w-24 rounded-md"></div>
					{:else}
						<div class="text-2xl font-bold text-gray-900 dark:text-white">
							{snapshot.summary.average_session_formatted}
						</div>
						<div class="mt-1 text-xs text-gray-500 dark:text-gray-400">
							{t('共', 'Total ')}
							{formatNumber(snapshot.summary.session_count)}
							{t('次会话', 'sessions')}
						</div>
					{/if}
				</div>
				<div
					class="relative z-10 flex h-12 w-12 items-center justify-center rounded-full bg-cyan-100 dark:bg-cyan-900/30"
				>
					<svg
			...
			<div class="metric-card relative flex items-center justify-between overflow-hidden p-6">
				<div class="relative z-10">
					<h3
						class="mb-2 text-xs font-semibold tracking-wider text-gray-500 uppercase dark:text-gray-400"
					>
						{t('最长单次会话', 'Longest Session')}
					</h3>
					{#if loading || !snapshot}
						<div class="skeleton-shimmer h-8 w-24 rounded-md"></div>
					{:else}
						<div class="text-2xl font-bold text-gray-900 dark:text-white">
							{snapshot.summary.longest_session_formatted}
						</div>
						<div class="mt-1 text-xs text-gray-500 dark:text-gray-400">
							{t('当前筛选区间内统计', 'Statistics within current range')}
						</div>
					{/if}
				</div>
				<div
					class="relative z-10 flex h-12 w-12 items-center justify-center rounded-full bg-blue-100 dark:bg-blue-900/30"
				>
					<svg
		...
		<div class="mb-8 grid grid-cols-1 gap-6 lg:grid-cols-3">
			<div class="surface-card flex h-96 flex-col p-6 transition-colors duration-200 lg:col-span-2">
				<div class="mb-6 flex items-center justify-between">
					<h2 class="text-lg font-semibold text-gray-800 dark:text-gray-100">
						{t('游玩时长 Top 游戏', 'Top Games by Playtime')}
					</h2>
				</div>
				<div class="min-h-0 flex-1">
					{#if loading || !snapshot}
						<div class="skeleton-shimmer h-full rounded-xl"></div>
					{:else}
			...
			<div class="surface-card flex h-96 flex-col p-6 transition-colors duration-200">
				<h2 class="mb-6 text-lg font-semibold text-gray-800 dark:text-gray-100">
					{t('游玩时长分布', 'Playtime Distribution')}
				</h2>
				<div class="relative min-h-0 flex-1">
					...
					{:else}
						...
						<div class="pointer-events-none absolute inset-0 flex items-center justify-center">
							<span
								class="rounded bg-white/80 px-2 py-1 text-xs font-medium text-gray-500 dark:bg-[#151926]/80 dark:text-gray-400"
								>{t('按时长', 'By Time')}</span
							>
						</div>
					{/if}
						...
						{#each snapshot.top_games.slice(0, 4) as game, i (game.game_id)}
							<div class="flex items-center">
								<span
									...
									style={`background-color: ${BAR_COLORS[i % BAR_COLORS.length]}`}
								></span>
		...
		<div class="surface-card mb-8 p-6 transition-colors duration-200">
			<h2 class="mb-4 text-lg font-semibold text-gray-800 dark:text-gray-100">
				{selectedDays === 1 ? t('小时趋势', 'Hourly Trend') : t('每日趋势', 'Daily Trend')}
			</h2>

			{#if selectedDays === 1 && snapshot}
				{@const hourlyInsights = getHourlyInsights(snapshot)}
				<div class="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
					<div class="subtle-panel px-3 py-2">
						<p class="text-[11px] tracking-wide text-cyan-700 uppercase dark:text-cyan-300">
							{t('峰值时段', 'Peak Slot')}
						</p>
						<p class="mt-1 text-sm font-semibold text-gray-900 dark:text-gray-100">
							{hourlyInsights.peakLabel} · {hourlyInsights.peakValue}
						</p>
					</div>
					<div class="subtle-panel px-3 py-2">
						<p class="text-[11px] tracking-wide text-indigo-700 uppercase dark:text-indigo-300">
							{t('当前时段', 'Current Slot')}
						</p>
						<p class="mt-1 text-sm font-semibold text-gray-900 dark:text-gray-100">
							{hourlyInsights.currentLabel} · {hourlyInsights.currentValue}
						</p>
					</div>
					<div class="subtle-panel px-3 py-2">
						<p class="text-[11px] tracking-wide text-amber-700 uppercase dark:text-amber-300">
							{t('小时均值', 'Hourly Avg')}
						</p>
						<p class="mt-1 text-sm font-semibold text-gray-900 dark:text-gray-100">
							{hourlyInsights.avgValue}
						</p>
					</div>
				</div>
			{/if}
		...
		<div class="table-shell transition-colors duration-200">
			<div
				class="flex items-center justify-between border-b border-gray-200 p-6 dark:border-gray-800"
			>
				<h2 class="text-lg font-semibold text-gray-800 dark:text-gray-100">
					{t('详细游戏统计', 'Detailed Game Statistics')}
				</h2>
				<span class="text-sm text-gray-500 dark:text-gray-400"
					>{t('按当前区间统计', 'Statistics in current range')}</span
				>
			</div>
			...
			<div class="overflow-x-auto">
				<table class="w-full text-left text-sm">
					<thead
						class="bg-gray-50 text-xs font-medium text-gray-500 uppercase dark:bg-white/5 dark:text-gray-400"
					>
						<tr>
					...
					<tbody class="divide-y divide-gray-200 dark:divide-gray-800">
						{#if loading || !snapshot}
							{#each [0, 1, 2, 3] as i (i)}
								<tr>
									<td class="px-6 py-4"><div class="skeleton-shimmer h-5 w-40 rounded"></div></td>
									<td class="px-6 py-4"><div class="skeleton-shimmer h-5 w-20 rounded"></div></td>
									<td class="px-6 py-4"><div class="skeleton-shimmer h-5 w-12 rounded"></div></td>
									<td class="px-6 py-4"><div class="skeleton-shimmer h-5 w-16 rounded"></div></td>
									<td class="px-6 py-4"><div class="skeleton-shimmer h-5 w-24 rounded"></div></td>
									<td class="px-6 py-4"
										><div class="skeleton-shimmer ml-auto h-5 w-12 rounded"></div></td
									>
								</tr>
							{/each}
						{:else if snapshot.top_games.length === 0}
							<tr>
								<td
									...
									>{t('当前区间没有可展示的数据', 'No data available in current range')}</td
								>
							</tr>
						{:else}
							{#each snapshot.top_games as game, index (game.game_id)}
								<tr class="transition-colors hover:bg-gray-50 dark:hover:bg-white/5">
									...
									<td class="px-6 py-4 text-gray-600 dark:text-gray-300"
										>{avgSessionText(game.seconds, game.session_count)}</td
									>
									<td class="px-6 py-4 text-gray-600 dark:text-gray-300"
										>{formatDate(game.last_played_at)}</td
									>
									<td class="px-6 py-4 text-right">
										<span
											class={`inline-flex items-center rounded px-2 py-0.5 text-xs font-medium ${trendClass(game.percentage)}`}
										>
